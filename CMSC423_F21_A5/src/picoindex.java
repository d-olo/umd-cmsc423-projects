...
/**
 * Takes a FASTA file and outputs a serialized index.
 * Taken from the code provided for project 3 for a suffix array.
...
public class picoindex {
	...
	public static void main (String[] args) {
		...
		try {
			ObjectOutputStream writer = new ObjectOutputStream(new FileOutputStream(args[1]));	
			for (int i = 0; i < sequence.length; i++) {
				...
				//Write serialized string to output file
				...
				writer.write("\n");*/
				writer.writeObject(suffixArray);
			}
			writer.close();
		} catch(IOException e) {
